...
右值的例子： 比如函数的返回值 int Foo(){ return 2; } 其中 Foo（）函数的返回值就是一个右值，因此 Foo() = 2是不允许的。 但如果情况如下所示：
...
```
这里 foo 返回一个引用。引用一个左值，因此可以赋值给它。 但如果a不是全局变量而是函数内的局部变量，这样是不可以的！！！ 因为这种情况下的返回值引用当函数返回时，栈的内容已经不存在了，引用会指向不知道的地方，很危险，

因此只能返回整个类型，但如果返回值是个类或者自定义结构体，本身占较大内存，在返回赋值时又进行了拷贝赋值，这样会很影响效率。 因此我们可以使用引用(因为是引用，就避免了拷贝)来接受这个返回值, const x& f = F()， 这里const是必须的
